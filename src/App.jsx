            ...
            <div className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
              ...
              <div className="space-y-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900">Katha Sthal</h4>
...
const PreviousWorksCarousel = () => {
  ...
  const nextSlide = () => setCurrent((prev) => (prev + 1) % previousWorksImages.length);
  const prevSlide = () => setCurrent((prev) => (prev - 1 + previousWorksImages.length) % previousWorksImages.length);
  ...
  if (previousWorksImages.length === 0) return null;
  ...
  return (
    <section className="w-full max-w-3xl mx-auto mt-16 mb-10 px-2 sm:px-0">
      ...
      <div className="relative rounded-xl overflow-hidden shadow-2xl bg-white">
        ...
        >
          {previousWorksImages.map((img, idx) => (
            <div
              key={idx}
              className="min-w-full flex-shrink-0 flex flex-col items-center justify-center bg-white"
            >
              <div className="w-full flex justify-center items-center bg-gray-50" style={{ minHeight: 220 }}>
                <img
                  src={img.src}
                  alt={img.alt}
                  className="w-full max-h-72 sm:max-h-96 object-cover object-center rounded-xl transition-all duration-300"
                  style={{
                    aspectRatio: "4/3",
                    maxWidth: "100%",
                    height: "auto",
              ...
              <div className="w-full px-2 py-3 text-center">
                <span className="block text-base font-semibold text-orange-700">
                  {imageCaptions[img.filename] || img.alt}
                </span>
                <span className="block text-xs text-gray-500 mt-1">
                  {/* Optionally, add a date or location here */}
                </span>
              </div>
            </div>
          ))}
